...
const DEFAULT_BULLET_DDAMAGE := 1
...
func _on_touch_damage_area_area_entered(area: Area2D) -> void:
	...
	var damaged := apply_damage(DEFAULT_BULLET_DDAMAGE)
	if damaged:
		bullet.queue_free()
...
func _update_touch_damage(delta: float) -> void:
	...
	if not is_instance_valid(touched_player):
		touched_player = null
		return
	if touch_damage_cooldown_left > 0.0: #TODO: 这里为什么要再次判断
		return
...
func _try_deal_touch_damage() -> void:
	...
	touched_player.apply_damaged(touch_damage)
...
func _die() -> void:
	...
	touch_damage_area.set_deferred("monitorable", true)
	_try_drop_pickup()
...
func _start_explosion_sequence() -> void:
	...
	if _play_death_sequence_animation(
		config.explosion_animation_name,
